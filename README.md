...
**侧边栏**按「发现 / 我的 / 正在播放 / 设置」分组，每个标签带图标并**标出数字键**：
...
> 侧边栏的**显示顺序**和数字键落点故意不同：数字键 1–9/0 始终对应
> `Home / 搜索 / 歌单 / 歌手 / 排行榜 / 云端 / 队列 / 歌词 / 封面 / 可视化`
> 这个原始顺序（改它会让肌肉记忆全乱），侧边栏只是换了个更好扫的排列，
> 所以每行前标了键名，照着按不会错。

**播放条**占 6 行：封面缩略图 + 歌名 + 歌手·专辑 + 进度条 + 下一首。
终端窄于 52 列时自动不画封面，把宽度让给文字。
